...
from utils.load import load_data, load_direct2v
...
class GifGeneration:
    ...
    def __call__(self, input_dict, save_gif=False, save_stats=False, metrics=['clip', 'ms_ssim', 'lpips', 'temporal_consistency_loss']):
        pipe = load_direct2v(self.device)
        stats_data = []


        for prompt, frame_prompts in tqdm(input_dict.items(), desc="Running tests", unit="test"):  
                        
            frames = self.pipe_call(frame_prompts, pipe)
    ...
    def pipe_call(self, frame_prompts, pipe):
        result = pipe(prompt=frame_prompts, video_length=len(frame_prompts)).images
